use toph::execution::{DigestMode, ExecutionMode, GifMode};
...
fn execute(mut args: pico_args::Arguments) -> Result<(), Box<dyn std::error::Error>> {
    toph::ALLOCATOR.reset();

    let default = toph::Options::default();

    if args.contains(["-h", "--help"]) {
        println!("toph v{}{}{}", toph::VERSION, HEADER, HELP);
        return Ok(());
    }

    if args.contains(["-V", "--version"]) {
        println!("toph v{}", toph::VERSION);
        return Ok(());
    }
    ...
    let glyphs = toph::data::GLYPHS;

    let options = toph::Options {
        width,
        height,
        headless,
        resizable,
    ...
    match args.free() {
        Ok(paths) => toph::init(&paths, options).map_err(|e| e.into()),
        Err(e) => {
